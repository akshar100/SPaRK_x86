...
asmlinkage long sys_sock_init_k()
{
    ...
    spark_registerMQ(SOCKET_CALL_CHANNEL2, CALL_CHANNEL2_SIZE);
    spark_registerMQ(SOCKET_TCP_RECV_CHANNEL2, TCP_RECV_CHANNEL2_SIZE);
    return 0;
}
/*****************************************************************************/
...
asmlinkage long sys_sock_listen_k(unsigned long port)
{
    ...
    else
    {
        ...
        ipc_listen_call[1] = 0x2;    //GOD id for GOS3 
        ...
        port_index++;
        spark_send(SOCKET_CALL_CHANNEL2, ipc_listen_call, sizeof(ipc_listen_call));
        return port;
    }
}
/*****************************************************************************/
...
asmlinkage long sys_sock_tcp_recv_k(unsigned long sockid, unsigned char *buffer, unsigned long size)
{
    ...
    */
    ...
    message_size = sys_receiveMQ_k(SOCKET_TCP_RECV_CHANNEL2, tcp_recv_data, size + 16);
    //spark_print("\n message_size");
    //spark_printLong(message_size);
    if(message_size != 0)
    {
        if(message_size != (size + 16))
            message_size = sys_receiveMQ_k(SOCKET_TCP_RECV_CHANNEL2, tcp_recv_data, message_size);
...
asmlinkage long sys_sock_tcp_send_k(unsigned long sockid, unsigned char *buffer, unsigned long size)
{
    ...
    *(((unsigned long *)tcp_send_data) + 1) = 0x2;    //GOD id for GOS3
    ...
    *(((unsigned long *)tcp_send_data) + 4) = sockid; //socket id
    ...
    //spark_print("\nIPC to COmm PArt\n");
    spark_send(SOCKET_CALL_CHANNEL2, tcp_send_data, 20+size);
...
asmlinkage long sys_sock_connect_k(unsigned long pip_addr, unsigned long port)
{
    ...
    else
    {
        ...
        ipc_connect_call[1] = 0x2;    //GOD id for GOS3 
        ...
        port_index++;
        spark_sockid++;
        spark_send(SOCKET_CALL_CHANNEL2, ipc_connect_call, sizeof(ipc_connect_call));
        return (ret);
    }
}
