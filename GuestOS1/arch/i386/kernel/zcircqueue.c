...
unsigned char tcp_send_packet_cq[APP_DATA_SIZE * APP_QUEUE_SIZE * MAX_OPEN_PORTS];
...
void init_cq_monitor(struct circ_queue_monitor * mycq)
{
    ...
    mycq->front = 0;
    mycq->count = 0;
    mycq->cqbuff = &(tcp_send_packet_cq[active_index]);
    memset((void *)tcp_send_packet_cq[active_index], 0, APP_DATA_SIZE * APP_QUEUE_SIZE);
