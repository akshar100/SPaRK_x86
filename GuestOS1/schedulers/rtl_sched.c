...
idle:
	...
	for (t = s->rtl_tasks; t; t = t->next) {
		if (now >= t->resume_time){
			...
			if (t->period != 0){
				// this is to sync the task time with the updation of time by Spark after deactivation of gos
	...
	if (!rtl_sigismember(&pthread_self()->pending, RTL_SIGNAL_READY)) 
	{
        ...
        //task_switch_performace.t2 = gethrtime();
	return mask;
}
/***************************************************************/
